...
void jstring2Char(JNIEnv* env, jstring inputJstring, char* outputCharstr) {
	const char *str = (*env)->GetStringUTFChars(env, inputJstring, 0);
 ...
 * 写入音频数据
 */
int Java_com_jqh_jni_nativejni_MediaPlayerJni_recordAduioPCMData(JNIEnv* env, jobject thiz, jbyteArray pcmData,jint len){
	...
	audioPkt.pts = writecount ; 
	if (ret < 0){
		LOGE("encoder failed");
	}
	if (got_frame==1){
		writecount++ ;
		LOGE("recordAduioPCMData %d\n",writecount);
...
/* 初始化ffmpeg                                                                     */
/************************************************************************/
int Java_com_jqh_jni_nativejni_MediaPlayerJni_recordMicInit(JNIEnv* env, jobject thiz, jstring* outputFile){
	...
	audioStream = addAudioStream(outputCtx, AV_CODEC_ID_SPEEX);
...
/* 关闭                                                                                                                                                                            */
/************************************************************************/
void Java_com_jqh_jni_nativejni_MediaPlayerJni_recordMicStop(JNIEnv* env, jobject thiz){
