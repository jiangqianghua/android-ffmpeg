    ...
    while(1)
    {
        ...
        if(framefinished)
        {
            ...
            if(acodeCtx->sample_fmt == AV_SAMPLE_FMT_S16)
            {
                ...
                int ret = sonicWriteShortToStream(tempoStream_, (short *)pAFrameCache->data[0], dst_buf_size/2*out_channels); 
                // 计算处理后的点数
            ...
            else
            {
                ...
                if(pSwrCtx)
                {
                    ...
                    int ret1 = sonicWriteShortToStream(tempoStream_, (short *)pAFrame->data[0], dst_buf_size/(2*out_channels)); 
                    // 计算处理后的点数  
                    int numSamples = dst_buf_size / (2*out_channels);  
                    if(ret1) {  
                    // 从流中读取处理好的数据  
