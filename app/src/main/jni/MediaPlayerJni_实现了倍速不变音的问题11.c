...
{
    isseekpos = pos ;
    LOGD("av_seek_frame set %lld" , isseekpos);
    return 0 ;
}
...
// 设置倍速速率
JNIEXPORT int Java_com_jqh_jni_nativejni_MediaPlayerJni_setSpeed(JNIEnv *env, jobject jobject,jdouble jspeed)
{
    speed = jspeed ;
    LOGD("set speed %lf",speed);
    return 0 ;
}
...
// 一些状态控制
    ...
    frame_timer = 0.0;
    LOGD("mutex lock -- num %d",lock_num);
}
// 初始化队列
...
int packet_queue_put(PacketQueue *q, AVPacket *pkt) 
{
    ...
    **/
    ...
    //对互斥锁上锁
    //LOGD("mutex lock ++ put %d",lock_num);
...
int packet_queue_get(PacketQueue *q,AVPacket *pkt, int block)  
{
    ...
    // }
    lock_num++;
    //LOGD("mutex lock ++ get %d",lock_num);
...
void *video_thread(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            LOGD("video_thread packet_queue_get --- no packet");
            LOGD("video_thread --- packet_queue_get 000");
            //usleep(200000);// 50ms   1ms = 1000us
            usleep(RESUME_WAITINH_TIME);
            continue ;
        }

        LOGD("video_thread --- packet_queue_get 222");
    ...
    LOGD("video_thread --- packet_queue_get 333");
    ...
    while(1)
    {
        ...
        if(framefinished)
        {
            ...
            if(acodeCtx->sample_fmt == AV_SAMPLE_FMT_S16)
            {
                ...
                int ret = sonicWriteShortToStream(tempoStream_, (short *)pAFrameCache->data[0], dst_buf_size); 
                // 计算处理后的点数
                LOGD("frame data[0] 11 %d",*(pAFrameCache->data[0]+1));  
                int numSamples = dst_buf_size / out_channels;  
                if(ret) {  
                // 从流中读取处理好的数据  
                   int new_buffer_size = sonicReadShortFromStream(tempoStream_, (short *)pAFrameCache->data[0], numSamples);  
                }  
            ...
            else
            {
                ...
                // int ret1 = sonicWriteShortToStream(tempoStream_, (short *)pAFrameCache->data[0], dst_buf_size); 
                // // 计算处理后的点数  
                // int numSamples = dst_buf_size / out_channels;  
                // if(ret1) {  
                // // 从流中读取处理好的数据  
                ...
                if(pSwrCtx)
                {
                    ...
                    // LOGD("frame data 11 %d",*(pAFrame->data[0]+1));
                    // LOGD("frame data 11-- %d",*(pAFrame->data[0]+1));
                    // //numSamples*sizeof(short)*stream->numChannels
                    // LOGD("frame data 11 size %d",dst_buf_size*out_channels);
                    
                    // LOGD("NO S16 +++ fmt=%d size=%d size=%d",acodeCtx->sample_fmt,dst_buf_size,pAFrame->linesize[0]);
                    int ret1 = sonicWriteShortToStream(tempoStream_, (short *)pAFrame->data[0], dst_buf_size/2); 
                    // 计算处理后的点数  
                    int numSamples = dst_buf_size / 2;  
                    if(ret1) {  
                    // 从流中读取处理好的数据  
                       int new_buffer_size = sonicReadShortFromStream(tempoStream_, (short *)pAFrame->data[0], numSamples);  
                    }  

                    LOGD("frame data 22 %d",*(pAFrame->data[0]+1));
                    //LOGD("is 16 bit");
                    // render to java
                    //LOGD("NO S16 --- fmt=%d size=%d ret1=%d",acodeCtx->sample_fmt,dst_buf_size,ret1);
...
void *recv_thread(void *arg)
{
    ...
    if(hasAudio())
    {
        ...
        int audioSampleRate = (int)(acodeCtx->sample_rate * speed) ;
        LOGD("InitAudio --- %d %d %lld channels=%d ",acodeCtx->sample_rate, audioSampleRate,acodeCtx->channel_layout,acodeCtx->channels);
        // 参数为采样率和声道数  
        tempoStream_ = sonicCreateStream(acodeCtx->sample_rate,acodeCtx->channels);
        // 设置速率
        sonicSetSpeed(tempoStream_, speed);  
        sonicSetPitch(tempoStream_, 1.0);
        sonicSetRate(tempoStream_, 1.0/speed);
        if(!hasVideo())
            (*env)->CallStaticVoidMethod(env, mediaPlayerJniClass, playMediaResult, 1,0,0,pFormatCtx->duration);
        (*env)->CallStaticVoidMethod(env, mediaPlayerJniClass, initAudio, audioSampleRate,JNI_TRUE,acodeCtx->channels > 1,4096);
        pthread_t tid2 ;
    ...
    while(1){
        ...
        //LOGD("av_seek_frame waiting %lld" , isseekpos);
        if(isseekpos != -1)
        {
            // 用户拖动进度条
            ...
            LOGD("av_seek_frame to -- %lld" , isseekpos);
            av_seek_frame(pFormatCtx, -1 , isseekpos * AV_TIME_BASE, AVSEEK_FLAG_BACKWARD);
            isseekpos = -1 ;
            packet_queue_clean(&audioq);
            packet_queue_clean(&videoq);
            // 音频相关
            audio_clock = 0.0;
            audio_buf_size = 0;
            audio_buf_index = 0;
