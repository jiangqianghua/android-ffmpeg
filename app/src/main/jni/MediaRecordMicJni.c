...
JNIEXPORT int Java_com_jqh_jni_nativejni_MediaPlayerJni_recordMicInit(JNIEnv *env,jobject jobject,
	...
	//[adpcm_swf @ 0x79650bc0] Sample rate must be 11025, 22050 or 44100
	mic_ofmt = av_guess_format(NULL, "file.flv", NULL); // file.aac(16000)  file.flv(11025, 22050(ok) or 44100)
	LOGD("222 %p",mic_ofmt);
	...
	mic_audio_st->time_base.num = 1 ; 
	mic_audio_st->time_base.den = mic_fps;
	...
	mic_pCodecCtx->sample_rate = 22050;
	...
	mic_pCodecCtx->framerate = (AVRational){mic_fps,1}; // 1秒15帧
	// 帧率的基本单位，分数表示
	mic_pCodecCtx->time_base = (AVRational){1,mic_fps};
	...
	if(!mic_codec){
		LOGD("avcodec_find_encoder error %d",mic_pCodecCtx->codec_id);
		return -1 ;
	}
...
JNIEXPORT int Java_com_jqh_jni_nativejni_MediaPlayerJni_recordAduioPCMData(JNIEnv *env,jobject jobject,
	...
	if(mic_got_frame == 1)
	{
		...
		//***********计算时间***********/
		AVRational time_base1= mic_audio_st->time_base; 
		AVRational time_base_q = { 1, AV_TIME_BASE }; 
		double frame_size = mic_audio_st->codec->frame_size;
		double sample_rate = mic_audio_st->codec->sample_rate;
		 //Duration between 2 frames (us)  
		int64_t calc_duration=(double)(AV_TIME_BASE) * (frame_size/sample_rate);
		mic_pkt.pts = av_rescale_q(mic_index*calc_duration, time_base_q, time_base1);
		mic_pkt.dts = mic_pkt.pts; 
		mic_pkt.duration = av_rescale_q(calc_duration, time_base_q, time_base1);
		mic_pkt.pos = -1;
		//------------------------------
		LOGD("frame_size %f sample_rate %f calc_duration %lld \n",frame_size,sample_rate,calc_duration);
		 mic_index++ ;
		 LOGD("Succeed to encode %d frame! \tsize:%5d pts=%lld dur=%d\n",mic_index,mic_pkt.size,mic_pkt.pts,mic_pkt.duration); 
