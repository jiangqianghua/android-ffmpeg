...
{	
	...
	if(frameFinished == 1)
	{
		...
		commonStatus->enc_pkt.pts = commonStatus->v_count *(time_base1.den)/((time_base1.num)*commonStatus->fps);
		commonStatus->enc_pkt.dts = commonStatus->enc_pkt.pts;
		commonStatus->enc_pkt.duration = (time_base1.den)/((time_base1.num)*commonStatus->fps);
...
/**
		...
		if(ji == 1)
		{
			AVRational sample_rate = pCodecCtx->framerate;
			// video
			calc_duration = (double)(AV_TIME_BASE) / av_q2d(sample_rate);
			commonStatus->enc_pkt.pts = av_rescale_q(commonStatus->v_count*calc_duration, time_base_q, time_base1);
			commonStatus->enc_pkt.dts = commonStatus->enc_pkt.pts; 
            commonStatus->enc_pkt.duration = av_rescale_q(calc_duration, time_base_q, time_base1);
            commonStatus->enc_pkt.pos = -1;
            commonStatus->v_count++ ;
            // 计算发送延迟
		...
		{
			double sample_rate = pCodecCtx->sample_rate;
			//audio
			calc_duration=(double)(AV_TIME_BASE) * (frame_size/sample_rate);
			commonStatus->enc_pkt.pts = av_rescale_q(commonStatus->v_count*calc_duration, time_base_q, time_base1);
			commonStatus->enc_pkt.dts = commonStatus->enc_pkt.pts; 
            commonStatus->enc_pkt.duration = av_rescale_q(calc_duration, time_base_q, time_base1);
            commonStatus->enc_pkt.pos = -1;
