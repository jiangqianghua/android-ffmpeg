    ...
    while(1)
    {
        ...
        if(framefinished)
        {
            ...
            if(acodeCtx->sample_fmt == AV_SAMPLE_FMT_S16)
            {
                ...
                // // 计算处理后的点数  
                // int numSamples = dst_buf_size / speed;  
                // if(ret) {  
                // // 从流中读取处理好的数据  
                ...
                //LOGD("is 16 bit");
                // render to java
                LOGD("S16 --- fmt=%d size=%d ",acodeCtx->sample_fmt,dst_buf_size);
            ...
            else
            {
                ...
                LOGD("NO S16 --- %d ",acodeCtx->sample_fmt);
                int nb_sample = pAFrameCache->nb_samples;
                int out_channels = acodeCtx->channels;
                ...
                if(pSwrCtx)
                {
                    ...
                    //LOGD("is 16 bit");
                    // render to java
                    LOGD("NO S16 --- fmt=%d size=%d ",acodeCtx->sample_fmt,dst_buf_size);
...
void *recv_thread(void *arg)
{
    ...
    if(hasAudio())
    {
        ...
        LOGD("InitAudio --- %d %d %lld",acodeCtx->sample_rate, audioSampleRate,acodeCtx->channel_layout);
        // 参数为采样率和声道数  
        tempoStream_ = sonicCreateStream(acodeCtx->sample_rate,acodeCtx->channels);
        // 设置速率
        sonicSetSpeed(tempoStream_, speed);  
        sonicSetPitch(tempoStream_, speed);
        sonicSetRate(tempoStream_, speed);
        if(!hasVideo())
