...
static int canvaWidth = 0; 
static int canvaHeight = 0; 
...
double nowTime();
void initAudioTrack(JNIEnv *env);
...
void Java_com_jqh_jmedia_JMediaPlayer_setPlaybackSpeed(JNIEnv *env, jobject jobject,jfloat jspeed)
{
    if(playStatus)
        playStatus->speed = jspeed ;
    // if(playStatus->play_status == PLAY_STATUS_START){
...
void Java_com_jqh_jmedia_JMediaPlayer_setCanvasSize(JNIEnv *env, jobject jobject,jint width, jint height)
{
    canvaWidth = width ; 
    canvaHeight = height ;
}
...
// 对图片进行缩放
jbyteArray Java_com_jqh_jmedia_JMediaPlayer_getScaleBitMap(JNIEnv *env, jobject jobj,jobject handler,uint32_t newWidth,uint32_t newHeight)
{
    void* _storedBitmapPixels = NULL;
    AndroidBitmapInfo _bitmapInfo;
    AndroidBitmap_getInfo(env, handler, &_bitmapInfo); 

    uint32_t oldWidth = _bitmapInfo.width;
    uint32_t oldHeight = _bitmapInfo.height;
    uint8_t* newBitmapPixels = malloc((newWidth * newHeight)*sizeof(uint8_t));// uint8_t[newWidth * newHeight];
    int len = newWidth * newHeight;
    int res = AndroidBitmap_lockPixels(env, handler, &_storedBitmapPixels);  
    if (_storedBitmapPixels == NULL) {  
        LOGD("fail to lock bitmap: %d\n", res);  
        return;  
    }  
    uint8_t* previousData  = _storedBitmapPixels;
    int x2, y2;
    int whereToPut = 0;
    int y = 0 ;
    ...
    AndroidBitmap_unlockPixels(env, handler);  
    jbyteArray newBitmapArray = (*env)->NewByteArray(env, len);
    (*env)->SetByteArrayRegion(env,newBitmapArray,0,len,newBitmapPixels);
...
void initAudioTrack(JNIEnv *env)
{
    //http://blog.csdn.net/dreamkid0924/article/details/53285085

    audioTrack = malloc(sizeof(AudioTrack));
    jclass audio_track_cls = (*env)->FindClass(env,"android/media/AudioTrack");
    jmethodID min_buff_size_id = (*env)->GetStaticMethodID(env,audio_track_cls,"getMinBufferSize","(III)I");
    audioTrack->buffer_size = (*env)->CallStaticIntMethod(env,audio_track_cls,min_buff_size_id,
                                        playStatus->sample_rate,2,2);
    LOGD("buffer_size:%d",audioTrack->buffer_size);
    jmethodID constructor_id = (*env)->GetMethodID(env,audio_track_cls,"<init>","(IIIIII)V");
    jobject audio_track_tmp = (*env)->NewObject(env,audio_track_cls,constructor_id,
                                3,playStatus->sample_rate_speed,2,2,audioTrack->buffer_size,1);

    // 如果其他地方要调用audio_track,必须关联全局，否则会提示
    //JNI ERROR (app bug): accessed stale local reference 0xbc00021 (index 8 in a table of size 8)
    audio_track = (jobject)(*env)->NewGlobalRef(env,audio_track_tmp);
    jmethodID method_play = (*env)->GetMethodID(env,audio_track_cls,"play","()V");
...
// 图片缩放，废弃
char* scaleBitMap(const uint8_t *previousData, int oldWidth, int oldHeight,int newWidth,int newHeight)
{
    char* newBitmapPixels = malloc(2*(newWidth * newHeight)*sizeof(uint8_t));// uint8_t[newWidth * newHeight];
    int len = newWidth * newHeight;
    int x2, y2;
    int whereToPut = 0;
    int y = 0 ;
...
void call_back_videodata(const uint8_t *pData, int width, int height,JNIEnv *env )
{
    ...
    // 对图片进行缩放处理
    //uint8_t * newData = scaleBitMap(pData,playStatus->videoWdith,playStatus->videoHeight,canvaWidth,canvaHeight);
    int len = 2*(canvaWidth*canvaHeight)*sizeof(uint8_t);
    //LOGD("video size -- %d width=%d height=%d",len,width,height);
    //LOGD("video size %d canvaWidth=%d canvaHeight=%d",len,canvaWidth,canvaHeight);
    //int len = 2*(width*height)*sizeof(uint8_t);
...
void call_back_aduiodata(AVFrame *frame,JNIEnv *env )
{
    ...
    jbyteArray audio_sample_array = (*env)->NewByteArray(env, frame->linesize[0]);
    (*env)->SetByteArrayRegion(env,audio_sample_array,0,frame->linesize[0],frame->data[0]);

    /**
    // 以下方法等同SetByteArrayRegion
    jbyte *sample_bytep = (*env)->GetByteArrayElements(env, audio_sample_array, NULL);
    //out_buffer的数据复制到sampe_bytep
    memcpy(sample_bytep, frame->data[0], frame->linesize[0]);
    //同步
    ...
    **/
    ...
    {
       // (*env)->CallIntMethod(env,audio_track,method_write,audio_sample_array,0,frame->linesize[0]);
        int i = 0 ; 
        for( i = 0 ; i < frame->linesize[0] ;  ){
            int result = (*env)->CallIntMethod(env,audio_track,method_write,audio_sample_array,0,frame->linesize[0] - i);
            LOGD("call_back_aduiodata reulst:%d",result);
            if(result > 0)
                i += result;
            else if(result == 0){
                usleep(1000);
    ...
    **/
    // java层回调
    (*env)->CallVoidMethod(env,jPlayMediaJniObj,jni_flush_audio_byte_data,audio_sample_array);
